...
export default function ApplicationForm() {
  ...
  const containerRef = useRef(null); // used to scroll to top when needed
  ...
  // validation returns errors object (so caller can immediately inspect first error)
  const getValidationErrors = (current = form) => {
    const e = {};
    if (!current.fullName?.trim()) e.fullName = "Full name is required";
  ...
  // Focus first invalid field & scroll top of container to make it visible
  const focusFirstError = (errs) => {
    setErrors(errs);
    const firstKey = Object.keys(errs)[0];
    if (!firstKey) return;
    // try to find input/select/textarea by name
    const el = containerRef.current?.querySelector(`[name="${firstKey}"]`);
    if (el) {
      // scroll container so element is visible near center
  ...
  const submitForm = async (e) => {
    ...
    try {
      // show overlay by setting loading true (prevents double submit)
      ...
      if (data.success) {
        // show success UI and scroll top so user sees it
        setSubmitted(true);
        containerRef.current?.scrollTo({ top: 0, behavior: "smooth" });
        ...
        // keep submitted message visible for ~3.5s
  ...
  // expose "Fill demo" action to mobile action bar
  const fillDemo = () =>
    setForm({
  ...
  const MobileActionBar = () => (
    <div className="fixed left-0 right-0 bottom-0 z-50 md:hidden bg-white/90 backdrop-blur border-t border-gray-200 p-3 flex items-center gap-3">
      ...
      <button
        onClick={submitForm}
        disabled={loading}
        className="flex-1 px-4 py-3 rounded-md bg-gradient-to-r from-[#7c4dff] to-[#fca532] text-white font-semibold text-sm inline-flex items-center justify-center gap-2"
      >
